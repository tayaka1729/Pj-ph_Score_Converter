...
const result_collr_0 = document.getElementById("result_collr_0");
const result_collr_1 = document.getElementById("result_collr_1");
const result_collr_2 = document.getElementById("result_collr_2");
result_collr_0.textContent = "";
result_collr_1.textContent = "";
result_collr_2.textContent = "";
...
fu=fu*2+1
score=(10/(fu))*(9*(pa+gr*0.65)+ma)
if (fu==pa || fu-1==pa){
    score=100
    document.getElementById("result_collr_2").textContent = "φ";
}else if(fu==ma || ma==fu-1){
    document.getElementById("result_collr_1").textContent = "V";
}else{
    if (score>=96) {
        hantei=0
    }else if (score>=92) {
    ...
    let hantei_0=["V","S","A","B","C","F"]
    result_collr_0.textContent=`${hantei_0[hantei]}`;
}
score=Math.round(10000*score);
result.textContent=`スコア: ${score}/`;
}